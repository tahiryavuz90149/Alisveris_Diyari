...
<body>
    ...
    <div class="formlar">
        ...
        <table>
            <thead>
            <h1> Silinecek Olan Kategori Bilgisini Giriniz</h1>
            <hr style=" width: 814px;  height: 1px;">
            
            </thead>
            <form action="kategori-sil.php" method="post">
                <tr>
                <td >Kategori Sil:</td>
                <td> <input type="text" name="kategori" placeholder="Kategori Girin" /></td>
                
                </tr>
                
                <tr>
                    <td> <button type="submit" class="btn btnÜrünEkle btn-outline-success">Kategori Sil</button></td>
                </tr>
            </form>
...
if(isset($_POST["kategori"]))  // eğer alan doluysa
{
  ...
  $Kategori=$_POST["kategori"];
   ...
   $sil = "DELETE FROM `kategori` WHERE kategori_adi = '".$Kategori."'";
   if($baglan->query($sil)===TRUE)
   {
      echo "<script> alert('Kategori bir şekilde silindi')</script>";
   }else
   {
    echo "<script> alert('Kategori silinirken bir hata oluştu.')</script>";
   }
}
